...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    try {
      ...
      // دقیقاً همان منطق امنیتی مدنظر شما
...
async function handlePublish(request: Request, env: Env) {
  // جلوگیری از کرش در صورت خالی بودن Body
  let body: any;
  try {
  ...
  // استفاده از prepare().run() به جای exec برای پایداری در D1
  ...
  // منطق اتصال به Hub (Master Worker)
...
async function getSessionToken(env: Env): Promise<string | null> {
  const row = await env.DB.prepare("SELECT value FROM settings WHERE key = 'master_jwt'").first<{value: string}>();
